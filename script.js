document.addEventListener('DOMContentLoaded', () => {
    ...
    // cria um array com 12 objetos 'carta' (sendo 1 imagem para cada 2 cartas)
    const cardArray = [
        {
            name: 'puppy1',
            img: 'img/img1.jpeg'
    ...
    let cardsChosen = []; // cria uma lista para armazenar as duas cartas escolhidas por rodada
    let cardsChosenId = []; // cria uma lista para armazenar os ids das duas cartas escolhidas por rodada
    let cardsWon = []; // cria uma lista com as cartas que já deram match (combinadas corretamente)

    // cria o "tabuleiro" do jogo: para cada um dos 12 objetos no array de cartas, será criada uma imagem e setado um endereço para ela (img/img7.jpeg). Também será setado um atributo personalizado chamado 'id' com o valor de 0 a 11 (já que são 12 objetos no total). Por fim, adiciona um evento de click e chama a função flipCard para cada pbjeto. O 'grid.appendChild(card)' é responsável por adicionar a img criada ao espaço reservado no html, o qual acessamos através da variável 'const grid'
